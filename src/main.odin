...
setup_pipeline :: proc() {
	...
	g.sampler = sdl.CreateGPUSampler(
		device = g.gpu,
...
load_model :: proc(mesh_file: string, texture_file: string) -> Model {
	...
	mem.copy(
		dst = transfer_mem,
		src = raw_data(vertices),
		len = vertices_byte_size,
	)
	mem.copy(
	...
	sdl.UnmapGPUTransferBuffer(device = g.gpu, transfer_buffer = transfer_buf)
	delete(indices)
	delete(vertices)
...
main :: proc() {
	...
	main_loop: for {
		...
		for sdl.PollEvent(&ev) {
			...
			case .KEY_UP:
				if !ui_input_mode do g.key_down[ev.key.scancode] = false
			case .MOUSE_MOTION:
